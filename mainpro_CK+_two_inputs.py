...
def train(epoch):
    ...
    for batch_idx, (inputs_face, inputs_saliency, targets) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        inputs, inputs_saliency, targets = Variable(inputs_face), Variable(inputs_saliency), Variable(targets)
        outputs = net(inputs)
        ...
        print('Current training accuracy: ', Train_acc)
...
def test(epoch):
    ...
    for batch_idx, (inputs_face, inputs_saliency, targets) in enumerate(testloader):
        ...
        if use_cuda:
            inputs_face, inputs_saliency, targets = inputs_face.cuda(), inputs_saliency.cuda(), targets.cuda()
            inputs_face, inputs_saliency, targets = Variable(inputs_face, volatile=True), Variable(inputs_saliency, volatile=True), Variable(targets)
